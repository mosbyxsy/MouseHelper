...
;  配置区 (按需修改)
; ================================================================
ScrollMode := "keys"   ; 初始模式: "keys"=按键(←/→)  "hscroll"=横向滚动
...
ScrollModeName() {
    global ScrollMode
    return (ScrollMode = "keys") ? "左右按键(滚轮=左右按键)" : "横向滚动(滚轮=横向滚动)"
}

; 当前侧键模式名称
SideBtnModeName() {
    global SideBtnMode
    return (SideBtnMode = "copy") ? "复制/粘贴(侧键直接使用)" : "设备默认(后退/前进)"
}

; 获取ModeTip
ModeTip() {
    global ScrollMode, SideBtnMode
    return "滚轮：" . ((ScrollMode = "keys") ? "左右按键" : "横向滚动") . " | 侧键：" . ((SideBtnMode = "copy") ? "复制/粘贴" : "设备默认")
}
...
ToggleScrollMode() {
    global ScrollMode
    MarkGestureUsed()
    ScrollMode := (ScrollMode = "keys") ? "hscroll" : "keys"
    A_IconTip := IconTip()
...
; 仅在"长按右键且非拖拽"或"宽限期内"时, 滚轮映射 ←/→
; (拖拽过程中滚轮保持原生行为; 宽限期用于滚动收尾)
#HotIf (GetKeyState("RButton", "P") and !isDragActive) or isGracePeriodActive
